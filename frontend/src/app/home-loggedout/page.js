    ...
    <div>
      <nav className="navbar">
        ...
        <div className="nav-links">
          ...
          <button className="logout-button" onClick={handleLogout}>Logout</button> {/* Clear logout option */}
        </div>
      </nav>
          ...
          {movies.filter(movie => movie.category === category && movie.title.toLowerCase().includes(searchTerm))
            ...
            <div key={index}>
              ...
              <iframe 
                ...
                allowFullScreen
              ></iframe>
              <button onClick={() => window.location.href = "/book-tickets"}>Book Movie</button>
            </div>
          ))}
        </div>
      ))}
